...
else db_url = config.db.localhost
...
MongoClient.connect(db_url,{ useUnifiedTopology: true, useNewUrlParser: true}, function(err, client) {
	// MongoClient.connect("mongodb://localhost/slashurl",{ useUnifiedTopology: true, useNewUrlParser: true}, function(err, client) {
	if (err) throw err;
			  ...
			  urlCollection.findOne({'long_url': longUrl}, function(err, doc) {
			  	...
			  	if (doc) {
			  		...
			  		countersCollection.findOne({_id: 'url_count'}, function(err, docs) {
			  			...
			  			countersCollection.updateOne({_id: 'url_count'}, {$inc: {val: 1}}, function(err) {
			  				if (err) throw err;
			  				shortUrl = config.webhost + baseHash.encode(newId);
			  				rClient.set(longUrl, shortUrl);
			  				urlCollection.insertOne({_id: newId, long_url: longUrl}, function(err) {
			  					if (err) throw err;
			  					client.close();
			  					res.send(shortUrl);
			  					return;
